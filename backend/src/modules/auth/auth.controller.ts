...
export class AuthController {
  ...
  @ApiResponse({
    status: 403,
    description: "CLIENT_ADMIN no puede crear otros admins",
  })
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "Asignar múltiples áreas a un usuario de una vez (solo OWNER)",
  })
  @ApiResponse({ status: 201, description: "Áreas asignadas exitosamente" })
  async assignMultipleAreas(
    @Param("userId") userId: string,
    @Body() body: ReplaceUserAreasDto,
    @CurrentUser() currentUser: any,
  ) {
    return this.userAreaService.replaceUserAreas(
      userId,
      body.areaIds,
      currentUser.userId,
    );
  ...
  @ApiOperation({
    summary:
      "Actualizar asignaciones centralizadas de usuario (clientes, proyectos, áreas)",
  })
  @ApiResponse({
    status: 200,
  ...
  async updateUserAssignments(
    @Param("userId") userId: string,
    @Body() body: UpdateUserAssignmentsDto,
    @CurrentUser() currentUser: any,
  ) {
    return this.userAssignmentService.updateAssignments(
      userId,
      body,
      currentUser.userId,
    );
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "Cambiar contexto de tenant sin logout (OWNER/PLATFORM_ADMIN)",
  })
  @ApiResponse({
    status: 200,
    description: "Contexto cambiado, nuevo token JWT emitido",
  })
  async switchTenant(
