...
export class BackupController {
  ...
  @ApiOperation({
    summary: "Crear backup completo de la base de datos (SOLO OWNER)",
  })
  ...
  @ApiOperation({
    ...
    description:
      "ADVERTENCIA: Esta operación sobrescribirá TODA la base de datos actual",
  })
  @ApiResponse({
    status: 200,
  ...
  @Get("list")
  @ApiOperation({
    summary: "Listar todos los backups disponibles (SOLO OWNER)",
  })
  ...
  async downloadBackup(
    ...
    const {
      ...
      filename: fname,
      size,
    } = await this.backupService.downloadBackup(filename, user);
