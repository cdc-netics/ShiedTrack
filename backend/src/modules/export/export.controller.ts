...
export class ExportController {
  ...
  @ApiOperation({
    summary: "Exportar lista de hallazgos a CSV con filtros (ANALYST+)",
  })
  async exportFindings(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="hallazgos_${Date.now()}.csv"`,
    );
    res.send(csv);
  }
  ...
  async exportProjectExcel(
    ...
    const stream = await this.exportService.exportProjectToExcel(
      projectId,
      user,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="proyecto_${projectId}_${Date.now()}.xlsx"`,
    );
  ...
  async exportProjectCSV(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="proyecto_${projectId}_${Date.now()}.csv"`,
    );
    res.send(csv);
  }
  ...
  @ApiOperation({
    summary: "Exportar proyecto a ZIP con evidencias (ANALYST+)",
  })
  async exportProjectZip(
    ...
    const zipStream = await this.exportService.exportProjectAsZip(
      projectId,
      user,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="project_${projectId}_${Date.now()}.zip"`,
    );
    zipStream.pipe(res);
  }
  ...
  async exportClientPortfolio(
    ...
    const zipStream = await this.exportService.exportClientPortfolio(
      clientId,
      user,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="client_${clientId}_${Date.now()}.zip"`,
    );
  ...
  async exportClientCSV(
    ...
    const csv = await this.exportService.exportClientPortfolioCSV(
      clientId,
      user,
    );
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="client_${clientId}_findings_${Date.now()}.csv"`,
    );
    res.send(csv);
  }
  ...
  async exportFindingPdf(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="finding_${findingId}.pdf"`,
    );
    res.send(buffer);
  }
  ...
  async exportProjectPdf(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="project_${projectId}.pdf"`,
    );
    res.send(buffer);
  }
  ...
  async exportFullBackup(@CurrentUser() user: any, @Res() res: Response) {
    const backup = await this.exportService.exportFullDatabaseBackup(user);

    res.setHeader("Content-Type", "application/json");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="shieldtrack_backup_${Date.now()}.json"`,
    );
  ...
  @ApiOperation({
    summary: "Ejecutar mongodump y generar archivo .tar.gz (SOLO OWNER)",
  })
  async createSystemBackup(@CurrentUser() user: any) {
